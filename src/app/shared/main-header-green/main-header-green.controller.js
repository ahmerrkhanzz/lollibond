(function() {
  ...
  angular
    ...
    .controller('mainHeaderGreenController', mainHeaderGreenController);

  /** @ngInject */
  function mainHeaderGreenController($scope, $rootScope, $http, $state, authService, bondService, searchService, baseService, lbUtilService, appDataFlowService, $uibModal) {
    var vm = this;
    ...
    function redirectToUser(item) {
      if (item.id === 0) {
        ...
        $state.go('corporate.profile', { uid: item.id });
      }
    }
    ...
    var stateChange = $rootScope.$on('$stateChangeStart', function(event, toState) {
      ...
      if (toState.name != 'corporate.search') {
        vm.appDataFlowService.searchQuery = '';
      }
    });
    ...
    // Open create company modal
      function openModal() {
        ...
        modalInstance.result.then(function (res) {
            $state.go('corporate.branches', { cid : res.id });
        });
      }
